...
inline IndexMapping3<> createCenteredIndexMapping( const float resolution, const Eigen::Vector3f &size, const Eigen::Vector3f &center = Eigen::Vector3f::Zero() ) {
	// from probeGenerator.cpp
	const Eigen::Vector3i halfExtent = ceil( size / (2.f * resolution) - Eigen::Vector3f::Constant( 0.5f ) );
	const Eigen::Vector3i count3 = 2 * halfExtent + Eigen::Vector3i::Constant( 1 );

	Eigen::Affine3f indexToPosition = Eigen::Translation3f( center ) * Eigen::Scaling( resolution ) * Eigen::Translation3f( -halfExtent.cast<float>() );
