...
public:
	Iterator3( const conceptIndexer3 &indexer ) : indexer( indexer ), index( 0 ), Index3Composition( indexer.getBeginCorner() ) {
	}
...
// main use: iterating over an indexer. Overloading the * operator to return index is the logical shorthand operation
template< typename conceptIndexer3 = SimpleIndexer3 >
class SubIterator3 : public Index3Composition {
	const conceptIndexer3 &indexer;
	Eigen::Vector3i beginCorner, endCorner;
	int index;

public:
	SubIterator3( const conceptIndexer3 &indexer, const Eigen::Vector3i &beginCorner, const Eigen::Vector3i &endCorner ) : indexer( indexer ), Index3Composition( beginCorner ), beginCorner( beginCorner ), endCorner( endCorner ), index( indexer.getIndex( beginCorner ) ) {
	}
...
struct Grid : SimpleIndexer3 {
	Eigen::Vector3f offset;
	float resolution;

	Grid( const Eigen::Vector3i &size = Eigen::Vector3i::Zero(), const Eigen::Vector3f &offset = Eigen::Vector3f::Zero(), float resolution = 0.0f ) : SimpleIndexer3( size ), offset( offset ), resolution( resolution ) {
	}
...
struct OrientedGrid : conceptIndexer3 {
	...
	OrientedGrid( const Indexer3 &indexer, const Eigen::Affine3f &indexToPosition ) : conceptIndexer3( indexer ), indexToPosition( indexToPosition ), positionToIndex( indexToPosition.inverse() ) {
	}
	...
	OrientedGrid permuted( const int permutation[3] ) const {
		...
		return OrientedGrid( Indexer3::permuted( permutation ), Eigen::Affine3f( permutedIndexToPosition ) );
	}
	...
	OrientedGrid<SubIndexer3> getSubGrid( const Eigen::Vector3i &beginCornerIndex, const Eigen::Vector3i &endCornerIndex ) const {
		return OrientedGrid<SubIndexer3>( SubIndexer3( beginCornerIndex, endCornerIndex ), indexToPosition );
	}
...
inline OrientedGrid<> OrientedGrid_from( const Eigen::Vector3i &size, const Eigen::Vector3f &offset, const float resolution ) {
	Eigen::Affine3f indexToPosition = Eigen::Translation3f( offset ) * Eigen::Scaling( resolution );
	return OrientedGrid<>( SimpleIndexer3( size ) , indexToPosition );
}

inline OrientedGrid<> OrientedGrid_from( const Grid &grid ) {
	return OrientedGrid_from( grid.size, grid.offset, grid.resolution );
}

typedef OrientedGrid<> SimpleOrientedGrid;
typedef OrientedGrid<SubIndexer3> SubOrientedGrid;
...
TEST( OrientedGrid, fromIdentity) {
	OrientedGrid<> grid = OrientedGrid_from( Vector3i::Constant(4), Vector3f::Zero(), 1.0 );
...
TEST( OrientedGrid, fromIdentityWithOffset) {
	...
	const OrientedGrid<> grid = OrientedGrid_from( Vector3i::Constant(4), offset, 1.0 );
...
TEST( OrientedGrid, fromOffsetAndResolution) {
	const Vector3f offset = Vector3f::Constant(1.0);
	const OrientedGrid<> grid = OrientedGrid_from( Vector3i::Constant(4), offset, 2.0 );
	EXPECT_EQ( grid.getPosition( Vector3i::Zero() ), offset );
...
TEST( OrientedGrid, permuted ) {
	...
	const Vector3f offset = Vector3f::Constant(1.0);
	const OrientedGrid<> grid = OrientedGrid_from( Vector3i::Constant(4), offset, 2.0 );
	const OrientedGrid<> permutedGrid = grid.permuted( permutation );
...
TEST( OrientedGrid, subGridSimple ) {
	const OrientedGrid<> grid = OrientedGrid_from( Vector3i::Constant(4), Vector3f::Zero(), 2.0 );
	const auto expandedGrid = grid.getSubGrid( Vector3i::Constant( -2 ), Vector3i::Constant(6) );
...
TEST( OrientedGrid, expandGridSimple ) {
	const OrientedGrid<> grid = OrientedGrid_from( Vector3i::Constant(4), Vector3f::Zero(), 2.0 );
	const auto expandedGrid = grid.getExpandedGrid( 3.0 );
...
TEST( Iterator3_Volume, coverageAndValidity ) {
		...
		Iterator3<SubIndexer3> iterator( indexer );
...
TEST( SubIterator3_Simple, simpleTest ) {
	SimpleIndexer3 indexer( Vector3i( 3,5,7 ) );
	SubIterator3<> iterator( indexer, Vector3i( 1, 1, 1), Vector3i( 3, 3, 3 ) );
...
TEST( SubIterator3_Volume, simpleTest ) {
	SubIndexer3 indexer( Vector3i( -1, -2, -3 ), Vector3i( 2,3,4 ) );
	SubIterator3<SubIndexer3> iterator( indexer, Vector3i( 0, 1, 1), Vector3i( 2, 3, 3 ) );
...
TEST( SubIterator3_Simple, coverageAndValidity ) {
	SimpleIndexer3 indexer( Vector3i( 3,5,7 ) );
	SubIterator3<> iterator( indexer, Vector3i( 0, 0, 0), Vector3i( 3, 5, 7 ) );
...
TEST( SubIterator3_Volume, coverageAndValidity ) {
	SubIndexer3 indexer( Vector3i( -1, -2, -3 ), Vector3i( 2,3,4 ) );
	SubIterator3<SubIndexer3> iterator( indexer, Vector3i( -1, -2, -3), Vector3i( 2, 3, 4 ) );
...
TEST( SubIterator3_Volume, index3ValueComparison ) {
	SubIndexer3 indexer( Vector3i( -1, -2, -3 ), Vector3i( 2,3,4 ) );
	SubIterator3<SubIndexer3> iterator( indexer, Vector3i( -1, -2, -3), Vector3i( 2, 3, 4 ) );
	Iterator3<SubIndexer3> refIterator( indexer );
