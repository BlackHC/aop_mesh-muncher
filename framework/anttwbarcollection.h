...
// first a custom structure implementation
/* idea:
 *  instead of writing one runtime definition, we write one runtime function that creates a structure for us and links it to a value object
 *  
 *  we can use a trait to link the original object to the AntTweakbar "view" factory
 *  the main issue is that anttwbargroup uses an incompatible mechanism to add items
 *  Im going to copy&paste together a new wrapper
 */
